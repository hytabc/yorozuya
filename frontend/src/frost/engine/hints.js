/**
 * 分级提示系统
 * 移植自 vrcWill/src/lib/engine/hints.ts。
 */
...
/** 根据卡关次数返回应当展示的提示层级。返回 0 表示"暂不提供提示"。 */
export function resolveHintTier(failCount) {
  let tier = 0
...
/** 取指定层级的提示。若未配置对应层级则向下回退。 */
export function getHint(level, tier) {
...
export function getRuleHint(level) {
  ...
  const parts = ['把碎片按你认为正确的顺序排好，然后让因果稳定。']
...
/** 从"最接近的结局"反推一句方向性提示。 */
export function directionalHint(level, nearestEndingId) {
