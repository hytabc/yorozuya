...
- `target`（可选）：`"focus"`（默认）作用于**当前焦点关系**；`"other"` 作用于**另一段活跃关系**
  （优先挑状态不是「砂糖」、亲密度最高的那段）并把它设为焦点——用于「同时多个重要人物」的多线剧情。
- 两种写法**可以同时出现**在同一 relationOp 上（先赋值再增减）。
- `type` 语义：
  - `spawn`：活跃关系数 < `MAX_ACTIVE_RELATIONS`（3）时创建一段新关系并成为焦点；
    **已满 3 段时忽略整个关系创建**（但数值键仍会应用到焦点关系上）。
  - `setState`：推进**目标关系**（默认焦点）的状态。若目标状态不在合法转移表（§4.3）中，
    **引擎拒绝转移并记录警告**，数值键照常应用；不满足 `vocab.relationGate` 的门槛时同样不推进状态、数值照常。
  - `end`：结束**焦点**关系；若还有别的活跃关系，焦点会自动切到那一段。`sugarCount` / `breakupCount` 已累计的不受影响。
  - `renew`：目标关系的 `freshness` 重置为 90、`realPressure` 清零，其余维度保留。
